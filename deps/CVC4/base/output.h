...
public:
    ...
    {
        return isConnected() ? *d_os : null_os;
    }

    std::ostream *getStreamPointer() const
    {
        return d_os;
    }

    template <class T> CVC4ostream &operator<<( T const &t ) CVC4_PUBLIC;
...
template <class T> CVC4ostream &CVC4ostream::operator<<( T const &t )
{
    if ( d_os != NULL )
    {
        if ( d_firstColumn )
...
public:
    explicit DebugC( std::ostream *os )
        : d_os( os )
    {
    }
...
public:
    explicit WarningC( std::ostream *os )
        : d_os( os )
    {
    }

    CVC4ostream operator()() const
    {
        return CVC4ostream( d_os );
    }
...
public:
    explicit MessageC( std::ostream *os )
        : d_os( os )
    {
    }
...
public:
    explicit NoticeC( std::ostream *os )
        : d_os( os )
    {
    }
...
public:
    explicit TraceC( std::ostream *os )
        : d_os( os )
    {
    }
...
public:
    ...
    explicit DumpOutC( std::ostream *os )
        : d_os( os )
    {
    }
    ...
    bool isOn( std::string tag ) const
    {
        return d_tags.find( tag ) != d_tags.end();
    }
...
#define WarningOnce                                                                                \
    ( !::CVC4::WarningChannel.isOn() || !::CVC4::WarningChannel.warnOnce( __FILE__, __LINE__ ) )   \
        ? ::CVC4::nullCvc4Stream                                                                   \
        : ::CVC4::WarningChannel
// This line is (manually) excluded because it clashes with the function
// of the same name found in `onnx.proto3.pb.h`.
// #  define Message (! ::CVC4::MessageChannel.isOn()) ? ::CVC4::nullCvc4Stream :
// ::CVC4::MessageChannel
