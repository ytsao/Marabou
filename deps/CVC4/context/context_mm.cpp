...
void ContextMemoryManager::newChunk()
{
    ...
    ++d_indexChunkList;
    Assert( d_chunkList.size() == d_indexChunkList ) << "Index should be at the end of the list";
...
void *ContextMemoryManager::newData( size_t size )
{
    ...
    if ( d_nextFree > d_endChunk )
    {
        ...
        res = (void *)d_nextFree;
        d_nextFree += size;
        AlwaysAssert( d_nextFree <= d_endChunk ) << "Request is bigger than memory chunk size";
    }
    Debug( "context" ) << "ContextMemoryManager::newData(" << size << ") returning " << res
                       << " at level " << d_chunkList.size() << std::endl;
