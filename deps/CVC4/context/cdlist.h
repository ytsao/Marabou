 ...
 *    operator=, and memcpy.
 */
template <class T, class CleanUpT, class AllocatorT> class CDList : public ContextObj
{
public:
...
protected:
    ...
    {
        Debug( "cdlist" ) << "copy ctor: " << this << " from " << &l << " size " << d_size
                          << std::endl;
    }
...
private:
    ...
    void grow()
    {
        if ( d_list == NULL )
        {
            ...
            Debug( "cdlist" ) << "initial grow of cdlist " << this << " level "
                              << getContext()->getLevel() << " to " << d_sizeAlloc << std::endl;
        ...
        else
        {
            ...
            Debug( "cdlist" ) << "2x grow of cdlist " << this << " level "
                              << getContext()->getLevel() << " to " << newSize << " (from "
                              << d_list << " to " << newList << ")" << std::endl;
            if ( newList == NULL )
            {
                throw std::bad_alloc();
            }
    ...
    ContextObj *save( ContextMemoryManager *pCMM ) override
    {
        ...
        Debug( "cdlist" ) << "save " << this << " at level " << this->getContext()->getLevel()
                          << " size at " << this->d_size << " sizeAlloc at " << this->d_sizeAlloc
                          << " d_list is " << this->d_list << " data:" << data << std::endl;
        return data;
    }
...
protected:
    ...
    {
        Debug( "cdlist" ) << "restore " << this << " level " << this->getContext()->getLevel()
                          << " data == " << data << " call dtor == " << this->d_callDestructor
                          << " d_list == " << this->d_list << std::endl;
        truncateList( ( (CDList<T, CleanUp, Allocator> *)data )->d_size );
        Debug( "cdlist" ) << "restore " << this << " level " << this->getContext()->getLevel()
                          << " size back to " << this->d_size << " sizeAlloc at "
                          << this->d_sizeAlloc << std::endl;
    }
...
class CDList<T, CleanUp, ContextMemoryAllocator<T>> : public ContextObj
{
    ...
    static_assert( sizeof( T ) == 0, "Cannot create a CDList with a ContextMemoryAllocator." );
};
