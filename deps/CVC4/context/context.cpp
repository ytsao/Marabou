...
void Context::push()
{
    Trace( "pushpop" ) << std::string( 2 * getLevel(), ' ' ) << "Push [to " << getLevel() + 1
                       << "] { " << this << std::endl;
...
void Context::pop()
{
    ...
    Trace( "pushpop" ) << std::string( 2 * getLevel(), ' ' ) << "} Pop [to " << getLevel() << "] "
                       << this << std::endl;
}
...
void ContextObj::update()
{
    ...
    Debug( "context" ) << "in update(" << this << ") with restore " << pContextObjSaved
                       << ": waypoint 1" << std::endl
                       << *getContext() << std::endl;
    ...
    Debug( "context" ) << "in update(" << this << "): next() == " << next() << std::endl;
    if ( next() != NULL )
    {
        Debug( "context" ) << "in update(" << this << "): next()->prev() == " << next()->prev()
                           << std::endl;
        next()->prev() = &pContextObjSaved->next();
        Debug( "context" ) << "in update(" << this << "): next()->prev() is now " << next()->prev()
                           << std::endl;
    }
    Debug( "context" ) << "in update(" << this << "): prev() == " << prev() << std::endl;
    Debug( "context" ) << "in update(" << this << "): *prev() == " << *prev() << std::endl;
    *prev() = pContextObjSaved;
    Debug( "context" ) << "in update(" << this << "): *prev() is now " << *prev() << std::endl;
    ...
    Debug( "context" ) << "after update(" << this << ") with restore " << pContextObjSaved << ":"
                       << std::endl
                       << *getContext() << std::endl;
}
...
void ContextObj::destroy()
{
    ...
    Debug( "context" ) << "before destroy " << this << " (level " << getLevel() << "):" << std::endl
                       << *getContext() << std::endl;
    ...
    , d_ppContextObjPrev( NULL )
{
    ...
    Debug( "context" ) << "create new ContextObj(" << this << " inCMM=" << allocatedInCMM << ")"
                       << std::endl;
    if ( allocatedInCMM )
    {
...
std::ostream &operator<<( std::ostream &out, const Context &context )
{
    ...
    for ( const_reverse_iterator i = context.d_scopeList.rbegin(); i != context.d_scopeList.rend();
          ++i, --level )
    {
        Scope *pScope = *i;
...
std::ostream &operator<<( std::ostream &out, const Scope &scope )
{
    ...
    Assert( pContextObj == NULL || pContextObj->prev() == &scope.d_pContextObjList );
