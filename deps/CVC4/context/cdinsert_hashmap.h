...
template <class Key, class Data, class HashFcn = std::hash<Key>> class InsertHashMap
{
private:
...
template <class Key, class Data, class HashFcn> class CDInsertHashMap : public ContextObj
{
private:
        ...
        , d_pushFronts( l.d_pushFronts )
    {
        Debug( "CDInsertHashMap" )
            << "copy ctor: " << this << " from " << &l << " size " << d_size << std::endl;
    }
    ...
    ContextObj *save( ContextMemoryManager *pCMM ) override
    {
        ...
        Debug( "CDInsertHashMap" )
            << "save " << this << " at level " << this->getContext()->getLevel() << " size at "
            << this->d_size << " d_list is " << this->d_insertMap << " data:" << data << std::endl;
        return data;
    }
...
protected:
    ...
    void restore( ContextObj *data ) override
    {
        Debug( "CDInsertHashMap" )
            ...
            << " data == " << data << " d_insertMap == " << this->d_insertMap << std::endl;
        ...
        Debug( "CDInsertHashMap" )
            << "restore " << this << " level " << this->getContext()->getLevel() << " size back to "
...
template <class Data, class HashFcn> class CDInsertHashMap<TNode, Data, HashFcn> : public ContextObj
{
    ...
    static_assert( sizeof( Data ) == 0, "Cannot create a CDInsertHashMap with TNode keys" );
};
