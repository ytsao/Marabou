...
#warning "Using the debug version of ContextMemoryManager, expect performance degradation"
 ...
 */
template <class T> class ContextMemoryAllocator
{
...
public:
    ...
    ContextMemoryAllocator( ContextMemoryManager *mm )
        : d_mm( mm )
    {
    }
    template <class U>
...
template <class T>
inline bool operator==( const ContextMemoryAllocator<T> &a1, const ContextMemoryAllocator<T> &a2 )
{
