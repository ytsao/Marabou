...
private:
    ...
    bool empty;
    ListNode( const T &t, ListNode<T> *n, bool e = false )
        : data( t )
        , next( n )
        , empty( e )
    {
...
template <class T> class List_iterator : public std::iterator<std::forward_iterator_tag, T>
{
    friend class List<T>;
...
private:
    ...
    List_iterator( const ListNode<T> *p )
        : pointee( p )
    {
    }
...
template <class T> void List<T>::append( const T &d )
{
    bck->checkConsistency();
    ...
    if ( uninitialized )
    {
        ...
        // head->data = d;
        head->empty = false;
        // Assert(tail == head); FIXME: do I need this because this list might be empty but append
        // to another one
        uninitialized = false;
    }
    else
...
template <class T> void List<T>::concat( List<T> *other )
{
    bck->checkConsistency();
...
template <class T> void List<T>::unconcat( List<T> *other )
{
